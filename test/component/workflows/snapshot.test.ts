...
describe('Component Test: Weekly Snapshot Creation', () => {
  ...
  beforeAll(async () => {
    ...
    const dbInfo = await query('SELECT current_database() as db_name, current_user as db_user, inet_server_addr() as server_addr');
  ...
  test('should create snapshot and reset sets to zero (happy path)', async () => {
    ...
    const setsBeforeSnapshot = await query(
      'SELECT number_of_sets FROM workout_day_sets WHERE id = $1',
      [chestSets.id]
    );
    ...
    const snapshotRecord = await query(
      'SELECT * FROM weekly_snapshots WHERE id = $1',
      [snapshot.id]
    );
    ...
    const dbDate = dbDateValue instanceof Date
      ? (() => {
    ...
    const setsAfterSnapshot = await query(
      'SELECT number_of_sets FROM workout_day_sets WHERE id = $1',
      [chestSets.id]
    );
  ...
  test('should replace existing snapshot for same week', async () => {
    ...
    const snapshots = await query(
      'SELECT * FROM weekly_snapshots WHERE routine_id = $1 AND week_start_date = $2',
      [routine.id, weekStartDateStr]
    );
    ...
    const oldSnapshotDays = await query(
      'SELECT * FROM snapshot_workout_days WHERE snapshot_id = $1',
      [snapshot1.id]
    );
